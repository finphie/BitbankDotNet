...
namespace BitbankDotNet.Tests.PublicApis
{
    ...
    public class BitbankRestApiClientGetTransactionsAsyncTest
    {
        ...
        public void HTTPステータスが200かつSuccessが1_Transactionを返す()
        {
            ...
            {
                var result = restApi.GetTransactionsAsync(default, default, default, default).GetAwaiter().GetResult();
        ...
        public void HTTPステータスが404またはSuccessが0_BitbankDotNetExceptionをスローする(HttpStatusCode statusCode, int success, int apiErrorCode)
        {
            ...
            using (var restApi = new BitbankRestApiClient(client))
            {
                var exception = Assert.Throws<BitbankDotNetException>(() =>
                    restApi.GetTransactionsAsync(default, default, default, default).GetAwaiter().GetResult());
